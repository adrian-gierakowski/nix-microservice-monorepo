...
let
  allOverlays =
    ...
    [
      ...
      (self: super: {
        services = self.kubenix.evalModules {
          modules = [
            (import ./modules/services.nix)
            ({ pkgs, lib, ... }: {
              config.services.server.package = pkgs.fileshare;
              config.services.server.runtimeConfigType = with lib.types; attrsOf int;
              config.services.server.runtimeConfig = { A = 1; };
            })
            ({ pkgs, lib, ... }: {
              config.services.server1.package = pkgs.fileshare;
              config.services.server1.runtimeConfig = { A = "1"; };
            })
            ({ pkgs, lib, ... }: {
